...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(`Ukuran file melebihi batas (${MAX_FILE_SIZE / (1024 * 1024)}MB)`)
  }

  // Unggah ke Media Library backend (POST /api/v1/admin/media/upload) dengan progress via XHR.
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    xhr.onload = () => {
      try {
        const json = JSON.parse(xhr.responseText) as { ok?: boolean; items?: { url: string }[]; message?: string }
        if (xhr.status >= 200 && xhr.status < 300 && json.items?.[0]?.url) {
          onProgress?.({ progress: 100 })
          resolve(json.items[0].url)
        } else reject(new Error(json.message ?? `Upload gagal (${xhr.status})`))
      } catch {
